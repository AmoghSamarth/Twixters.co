...
export function Pricing() {
  return <section id="pricing" aria-labelledby="pricing-heading" className="px-5 py-24 sm:px-8 sm:py-32">
    ...
    /* Included column */
          ...
          <div className="rounded-[22px] bg-surface p-6 shadow-float sm:p-8">
            ...
            <ul className="mt-5 grid gap-3.5">
              {pricing.included.map((item) => <li key={item} className="flex items-start gap-3">
                  <PlusInCircle />
    ...
    /* Capability grid — plain editorial rows, never cards. */
      ...
      <Reveal delay={60} className="mx-auto mt-11 max-w-[1200px] sm:mt-14">
        <ul className="grid gap-x-10 gap-y-5 sm:grid-cols-2 lg:grid-cols-3">
          {capabilities.map((item) => <li key={item} className="flex items-center gap-3">
              <svg
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
