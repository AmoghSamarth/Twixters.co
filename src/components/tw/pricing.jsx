      ...
      <Reveal delay={90} className="mx-auto mt-14 max-w-[1200px]">
        <div className="grid gap-10 rounded-[28px] bg-gradient-to-b from-[#e8e8e8] to-[#f7f7f7] p-6 shadow-lift sm:rounded-[36px] sm:p-10 lg:grid-cols-[40%_1fr] lg:gap-14">
          ...
          <div className="flex flex-col justify-between">
            <div>
              {/* Interactive plan switcher toggle */}
              <div className="inline-flex items-center gap-1 rounded-pill bg-[#dcdcdc] p-1">
                <button
                  type="button"
                  onClick={() => setIsCustom(false)}
                  className={`rounded-pill px-4 py-2 text-[13px] font-semibold tracking-tight transition-all duration-300 ${
                    !isCustom
              ...
              <div className="mt-8 transition-opacity duration-300">
                {!isCustom ? (
                  ...
                  <p className="flex flex-wrap items-baseline gap-2">
                    <span className="text-[19px] font-normal text-ink-faint">from</span>
                    <span className="text-[clamp(2.6rem,7vw,3.6rem)] leading-none font-bold tracking-[-0.035em] text-ink">
                      $11,500
                    </span>
                  </p>
          ...
          <div className="rounded-[22px] bg-surface p-6 shadow-float sm:p-8">
            ...
            <ul className="mt-5 grid gap-3.5 transition-all duration-300">
              {(!isCustom ? pricing.included : customIncluded).map((item) => (
            ...
            <div className="mt-8 border-t border-hair-soft pt-7">
              <p className="text-[15px] leading-[1.6] text-ink-muted">
                {!isCustom
                  ? pricing.bio.text
                  : "Effortless process. Exceptional results. Working with Raj and the Twixters team felt like having an in-house elite design partner on speed dial."}
              </p>
