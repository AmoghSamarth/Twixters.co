...
export function Expertise() {
  ...
  // Active indices: start with reference states (Brand Assets & Hoardings) or step on scroll
  const [activeLeft, setActiveLeft] = useState(6);
  const [activeRight, setActiveRight] = useState(2);
  const activeLeftRef = useRef(6);
  const activeRightRef = useRef(2);

  // Manual click handler (locks out scroll hijacking for 3.5s)
  ...
  // When scrolled, advance images & active services smoothly
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (!userInteractedRef.current) {
        // Map progress across the 7 service items [0..6]
        const nextIdx = Math.min(6, Math.max(0, Math.floor(progress * 7)));
        if (nextIdx !== activeLeftRef.current) {
          activeLeftRef.current = nextIdx;
          setActiveLeft(nextIdx);
        }
        if (nextIdx !== activeRightRef.current) {
          activeRightRef.current = nextIdx;
          setActiveRight(nextIdx);
        }
      }
    };
  ...
  return (
    /* ─── Outer Track: Sticky Pinning Track so scrolling changes images smoothly ─── */
    <div ref={outerTrackRef} className="relative lg:min-h-[260vh]">
      <section
        id="expertise"
        aria-label="What we design"
        className="relative lg:sticky lg:top-0 w-full min-h-screen lg:h-screen flex flex-col justify-between overflow-hidden px-5 sm:px-8 lg:px-12 xl:px-16 2xl:px-20 pt-14 sm:pt-16 lg:pt-12 pb-7 lg:pb-8 select-text border-t border-neutral-300/80 shadow-[inset_0_20px_40px_-15px_rgba(0,0,0,0.06)]"
        style={{
          background: "radial-gradient(ellipse at 50% 48%, #eae8e1 0%, #f4f3ee 65%, #f6f5f0 100%)",
        }}
      >
          ...
          <div className="relative my-auto py-2">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-12 xl:gap-18 items-center">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[auto_1fr] gap-x-6 xl:gap-x-10 gap-y-6 lg:items-center">
                ...
                {/* Horizontal Image Gallery Deck: Decluttered (3 Cards Visible) */}
